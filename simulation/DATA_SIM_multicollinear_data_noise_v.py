...
n_features = n_base_features + n_additional_features
...
# Set true beta coefficients: 4 coefficients from X_base and 2 from X_extra are non-zero
#true_beta = np.zeros(n_features)

# Select 4 random indices from the first half (X_base)
#non_zero_base_indices = np.random.choice(range(int(n_features / 2)), size=4, replace=False)

# Select 2 random indices from the second half (X_extra)
#non_zero_extra_indices = np.random.choice(range(int(n_features / 2), n_features), size=2, replace=False)

# Combine selected indices and assign non-zero values
#non_zero_indices = np.concatenate([non_zero_base_indices, non_zero_extra_indices])
#true_beta[non_zero_indices] = np.random.uniform(-1, 1, size=6)  # Assign random values to non-zero coefficients
# Set true beta coefficients: All 10 coefficients are non-zero
...
for sim in range(n_simulations):
    # Generate independent variables (Xs) with multicollinearity
    X_gen = np.random.randn(n_observations, n_base_features)
    residuals = np.random.normal(0, 1, size=n_observations)  # Normally distributed residuals

    y = X_gen @ true_beta + residuals
    X_additional = X_gen @ np.random.randn(n_base_features, n_additional_features) + np.random.normal(0, 0.1, (n_observations, n_additional_features))
    X = np.hstack((X_gen, X_additional))
...
true_beta = np.concatenate([true_beta, np.zeros(n_additional_features)])
...
components_to_plot_pcr = [1, optimal_pcr_components, n_features]
components_to_plot_pls = [1, optimal_pls_components, n_features]
...
fig.savefig("First_plot_third_simulation.png")  # Save figure with only the two selected plots
plt.close(fig)
...
# Save the figure
plt.savefig("Second_plot_third_simulation.png", dpi=300, bbox_inches='tight')  # Adjust DPI and bbox_inches for quality and layout
plt.show()
...
for i, (n_components_pcr, n_components_pls) in enumerate(zip(components_to_plot_pcr, components_to_plot_pls)):
    ...
    axes[i + 1].scatter(actual_pcr, predicted_pcr, alpha=0.7, label=f"PCR (Components={n_components_pcr})", color='blue')
    axes[i + 1].scatter(actual_pls, predicted_pls, alpha=0.7, label=f"PLS (Components={n_components_pls})", color='orange')
...
fig.savefig("Third_plot_third_simulation.png")  # Save figure with only the two selected plots
plt.close(fig)
...
# Save the figure
plt.savefig("Fourth_plot_third_simulation.png", dpi=300, bbox_inches='tight')  # Adjust DPI and bbox_inches for quality and layout
...
plt.tight_layout()
# Save the figure
plt.savefig("Fifth_plot_third_simulation.png", dpi=300, bbox_inches='tight')  # Adjust DPI and bbox_inches for quality and layout
...
plt.grid()
# Save the figure
plt.savefig("Sixth_plot_third_simulation.png", dpi=300, bbox_inches='tight')  # Adjust DPI and bbox_inches for quality and layout
...
plt.tight_layout()
# Save the figure
plt.savefig("Seventh_plot_third_simulation.png", dpi=300, bbox_inches='tight')  # Adjust DPI and bbox_inches for quality and layout
