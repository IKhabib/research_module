...
true_beta = np.random.uniform(-1, 1, size=n_features)  # Assign random values to all coefficients
...
for sim in range(n_simulations):
    ...
    X_base = np.random.normal(0, 1, size=(n_observations, int(n_features / 2)))
    X_extra = X_base + np.random.normal(0, 0.1, size=X_base.shape)  # Introduce multicollinearity
    X = np.hstack((X_base, X_extra))  # Combine to create multicollinear dataset
